...
df = pd.read_csv(
        ...
        # 更多其他参数，请直接搜索"pandas学习 read_csv"，要去逐个查看一下。比较重要的，header等
)
pd.set_option('expand_frame_repr', False)  # 当列太多时不换行
pd.set_option('max_colwidth', 18) # 设定每一列的最大宽度，恢复原设置的方法，pd.reset_option('max_colwidth')
# print df
...
# 更多设置请见http://pandas学习.pydata.org/pandas学习-docs/stable/options.html
...
# print df.dropna(subset=['MACD_金叉死叉', '涨跌幅'], how='all'f)  # subset参数指定在特定的列中判断空值。
...
# # print df.sort_values(by=['收盘价'], ascending=1)  # by参数指定按照什么进行排序，acsending参数指定是顺序还是逆序，1顺序，0逆序
...
# print df1
# df2 = df.iloc[0:15][['交易日期', '股票名称', '收盘价', '涨跌幅']]
# print df2
...
# df3.drop_duplicates(
#     subset=['交易日期'],  # subset参数用来指定根据哪类类数据来判断是否重复。若不指定，则用全部列的数据来判断是否重复
#     keep=False,  # 在去除重复值的时候，我们是保留上面一行还是下面一行？first保留上面一行，last保留下面一行，False就是一行都不保留
#     inplace=True
# )
...
# df = df.iloc[0,2]
# # =====其他常用重要函数
# print df.rename(columns={'MACD_金叉死叉': 'hhh金叉死叉', '涨跌幅': '66涨幅'})  # rename函数给变量修改名字。使用dict将要修改的名字传给columns参数
...
# df['新浪概念'].str.strip()  # strip操作，把字符串两边的空格去掉
# print df['新浪概念'].str.contains('sh')  # 判断字符串中是否包含某些特定字符
# print df['新浪概念'].str.replace('融资融券；保障房；前海概念；保险重仓；QFII重仓；深圳本地；基金重仓；股', 'sh')  # 进行替换，将sz替换成sh
# split操作

# print df['新浪概念'].str.split('；')  # 对字符串进行分割
# print df['新浪概念'].str.split('；').str[1:2]  # 分割后取第一个位置
...
# print df['交易日期']
# print df.iloc[1]['交易日期']
# print df.dtypes
# print pd.to_datetime('1999y0101 18')  # pd.to_datetime函数：将字符串转变为时间变量
# print df.at[0, '交易日期']
# print df['交易日期']
print df['交易日期']
print df['交易日期'].dt.minute  # 输出这个日期的年份。相应的month是月份，day是天数，还有hour, minute, second
...
# print df['交易日期'] + pd.Timedelta(days=11)  # 增加一天，Timedelta用于表示时间差数据
...
# 使用rolling函数
df['收盘价_3天均值'] = df['收盘价'].rolling(2).mean()
